...
#     img = cv2.imread('/Users/minghanchan/Desktop/Internships/beeware-tutorial/helloworld/download,jpg.jpeg')
...
def getOutlineImg(img):
    ...
    #use higher sigma if contrast high
    # sigma = 0.99
    # v = np.median(img)
    ...
    # return cv2.Canny(img,lower, upper)  # todo: can be optimised later 
    return cv2.Canny(img,25,55) #best values so far
    #return cv2.Sobel(img, -1, 1, 1) #super noisy
    # return cv2.Scharr(img, -1,0,1) #super noisy
    # return cv2.Laplacian(img,-1, ksize=5) #doesnt identify wall well
    #original thresholds 50,200
    #10,100 seems better
...
def saveImage(img_name, img):
    ...
    cv2.imwrite( "raw_frame" + img_name, img)
...
def changeColor(img, position, new_color, pattern_image):
